...
7. Red: entregar el resultado a Solicitudes confirma la transición y el evento terminal correspondiente. Green: cargar su agregado por ID, traducir el evento a `ResultadoEvaluacion` propio de Solicitudes y aplicar ese objeto mediante `aplicar_resultado_evaluacion`, guardar y registrar sus pendientes como salidas. Una solicitud inexistente genera un error explícito sin crearla. Una evaluación ajena, inválida o diferente de la inicial no cambia estado ni salidas. Repetir la evaluación original no cambia versión, fecha, evaluación ni eventos. Conservar las defensas del agregado y no recalcular la política en este handler.
...
Reglas del Partner calcula la evaluación; Solicitudes aplica el resultado a su propio estado. El bus entrega `ReglasDePartnerEvaluadas` a `AplicarResultadoEvaluacionHandler`, que traduce sus campos y enumeraciones a `ResultadoEvaluacion` de Solicitudes e invoca `solicitud.aplicar_resultado_evaluacion(resultado, ...)`. La traducción conserva ID e instante del evento de origen, IDs de evaluación, solicitud, partner y política, versiones, admisibilidad y red o motivo. El objeto propio no hereda de `EventoDominio`. Tanto el agregado como sus eventos finales conservan ese resultado, sin importar tipos de Reglas. No se añade un comando intermedio, otro handler ni otro bus. Esta llamada interna al agregado no recalcula políticas ni invoca handlers de Reglas.
...
| Misma ubicación | `aplicar_resultado.py` | `AplicarResultadoEvaluacionHandler`: recibir `ReglasDePartnerEvaluadas` desde el bus, traducirlo a los tipos propios y aplicar su resultado al agregado de Solicitudes, guardar y preparar salidas. |
...
| `src/solicitudes_partner/modulos/solicitudes/dominio/` | `objetos_valor.py` existente | Añadir `ResultadoEvaluacion` y sus enumeraciones propias `Admisibilidad`, `TipoRedProveedores` y `MotivoRechazo`; validar identidad, fechas, versiones y coherencia del resultado. |
...
| Misma ubicación | `test_aplicacion_resultado_evaluacion.py` | Transición por handler, resultado ajeno, ausencia y repetición. |
...
Los eventos de dominio se definen y se importan directamente desde `dominio/eventos.py`, sin fachadas de reexportación. Las enumeraciones se definen y se importan desde `objetos_valor.py`. El dominio de Solicitudes no importa ningún tipo de Reglas. Su handler receptor conoce el evento compartido y traduce a los tipos locales; las enumeraciones locales se mantienen explícitamente como parte del modelo consumidor. En las demás fronteras solo se permite importar los eventos compartidos y sus enumeraciones, nunca entidades, políticas, servicios o repositorios ajenos. La recepción del registro en Reglas conserva su diseño actual; este cambio no aísla el dominio de Reglas respecto de Solicitudes. Los esquemas de integración y adaptadores de transporte se implementarán en infraestructura durante 05; no duplicar ahora eventos ni enumeraciones. Los puertos comunes solo conocen mecanismos generales. Las pruebas conservan los modelos reales y sustituyen sus dependencias de I/O.
...
- **Aislamiento de Solicitudes:** cargar su dominio con los imports de Reglas bloqueados; verificar conversión a enumeraciones propias, conservación de trazabilidad, reentregas equivalentes e incompatibles y eventos terminales con el resultado local. `aplicar_resultado_evaluacion` es un método de instancia.
