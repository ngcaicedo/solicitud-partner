...
Estado: implementado y verificado localmente. Dependencias: ninguna. [Evidencia y límites del cierre](evidencia-01-base-tecnologica.md).

Ajuste durante implementación: TestClient de la versión resuelta de Starlette requiere `httpx2`, que sustituye al `httpx` inicialmente propuesto. El tipado incluye también `scripts`; las verificaciones locales usan las variantes `uv run --locked` de los comandos siguientes.
...
|---|---|---|
| Python | 3.12 como candidato; fijarlo tras comprobar instalación e importación de una versión candidata del cliente Pulsar en la plataforma de desarrollo | Evitar incompatibilidad tardía de wheels |
| uv | Gestión de Python, dependencias, entorno y lockfile | Un entorno de desarrollo reproducible |
...
Las carpetas de módulos, capas y suites se nombran en español, sin tildes y con `snake_case`, según la [convención común de los planes](README.md#convención-de-nombres-para-todos-los-planes). Se conservan las convenciones técnicas `src`, `api`, `config`, `seedwork` y `tests`, y la ruta documental ya acordada `docs/plans`. La base tecnológica implementada conserva sus identificadores actuales. Para los nuevos incrementos rige la convención común actualizada: identificadores propios en español, con el vocabulario del Event Storming.
...
3. Configurar pytest, clasificación de suites, Ruff y mypy. Registrar en README los comandos canónicos y la sincronización con lockfile.
...
9. Documentar instalación, ejecución local y límites de lo implementado. Documentar la ejecución local de unitarias, API aislada, lint y tipado.
...
Este proceso permanece en ejecución. Consultar `GET /health/live` desde otra terminal, registrar código y cuerpo, y detener el servidor con Ctrl+C. No tratar este comando como una verificación que termina por sí misma.
...
- Pruebas unitarias y API aislada, lint, formato y typecheck verdes localmente. Conservar los comandos ejecutados y sus resultados, sin presentar comprobaciones pendientes como aprobadas.
